...
const chatReceive = (userMsg) => {
  ...
  const API_KEY = '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'
...
const getCatImg = () => {
  axios({
    ...
    .then(res => {
      ...
      // addImg(res.data[0].url)
      // 강아지
      console.log(res.data.message)
      addImg(res.data.message)
    })
    .catch(err => {
      console.log(err)
    })
...
const chatSubmit = () => {
  ...
  const mySiteList = ['ssafy', '싸피']
  const lottoWordList = ['lotto', '로또']
  const catWordList = ['cat', '고양이']

  if (mySiteList.includes(value)) {
    myServer()
  } else if (lottoWordList.includes(value)) {
    getLotto()
  } else if (catWordList.includes(value)) {
    // const src = 'https://img3.daumcdn.net/thumb/R658x0.q70/?fname=https://t1.daumcdn.net/news/202211/24/catlab/20221124080217131azjg.jpg'
    // addImg(src)
    getCatImg()
  } else {
    chatReceive(value)
